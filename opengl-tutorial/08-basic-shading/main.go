...
// Tutorial 08 - Basic shading ported from
...
func main() {
	...
	window, err := glfw.CreateWindow(1024, 768, "Tutorial 08", nil, nil)
	if err != nil {
		panic(err)
	}
	...
	gl.BufferData(gl.ARRAY_BUFFER, len(normals)*4*3, gl.Ptr(normals), gl.STATIC_DRAW)
	...
	for window.GetKey(glfw.KeyEscape) != glfw.Press && !window.ShouldClose() {
		...
		// Draw the triangle !
		gl.DrawArrays(gl.TRIANGLES, 0, int32(len(vertices))) // 12*3 indices starting at 0 -> 12 triangles
